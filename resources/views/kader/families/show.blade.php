...
<!-- Family Profile Card -->
<div class="bg-white border border-slate-100 rounded-3xl p-5 sm:p-6 shadow-xs relative overflow-hidden mb-6">
    <div class="absolute -top-12 -right-12 w-32 h-32 bg-emerald-50 rounded-full blur-2xl"></div>
    <div class="relative z-10 grid grid-cols-1 md:grid-cols-4 gap-4 sm:gap-6 items-start md:items-center">
        <div class="flex items-center gap-3.5 md:col-span-2">
            <div class="bg-emerald-50 text-emerald-600 h-12 w-12 sm:h-14 sm:w-14 rounded-2xl flex items-center justify-center font-bold text-base sm:text-lg border border-emerald-100 shrink-0">
                {{ substr($family->kepala_keluarga, 0, 2) }}
            </div>
            <div class="min-w-0">
                <h2 class="text-lg sm:text-xl font-bold text-slate-800 leading-snug">Keluarga Bapak {{ $family->kepala_keluarga }}</h2>
        ...
        <div class="md:col-span-2 text-left md:text-right pt-3 md:pt-0 border-t md:border-t-0 border-slate-100">
            <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Alamat Rumah</span>
            <span class="text-xs text-slate-600 font-medium mt-0.5 inline-block leading-relaxed">{{ $family->alamat }}</span>
        </div>
    </div>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <!-- List Members Card -->
        <div class="bg-white border border-slate-100 rounded-3xl p-5 sm:p-6 shadow-xs">
            <h3 class="text-base font-bold text-slate-800 mb-6 flex items-center gap-2">
                <svg class="w-5 h-5 text-emerald-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path></svg>
                Daftar Anggota Keluarga
            </h3>

            <div class="space-y-4">
                @forelse($members as $m)
                    <div class="bg-slate-50/60 border border-slate-100 rounded-2xl p-4 space-y-3 transition hover:border-emerald-200">
                        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                            <div class="flex items-start gap-3">
                                <div class="bg-emerald-50 text-emerald-600 h-10 w-10 rounded-full flex items-center justify-center font-bold text-xs shrink-0 mt-0.5">
                                    {{ substr($m->nama, 0, 2) }}
                                </div>
                                <div class="space-y-1 min-w-0">
                                    <h4 class="font-bold text-slate-800 text-xs flex flex-wrap items-center gap-1.5">
                                        <span>{{ $m->nama }}</span>
                                        <span class="text-[9px] font-semibold text-slate-500 bg-slate-200/60 px-1.5 py-0.5 rounded-full">
                                    ...
                                    <span class="inline-block text-[9px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-100 px-2 py-0.5 rounded-full">
                                        {{ $m->kategori_formatted }}
                                    </span>
                                </div>
                            </div>

                            <div class="flex flex-wrap items-center gap-2 pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-100/80 w-full sm:w-auto">
                                @if($m->status_verifikasi === 'pending')
                                    <!-- Verify Actions -->
                                    <form action="{{ route('kader.members.verify', [$m->id, 'disetujui']) }}" method="POST" class="flex-1 sm:flex-none">
                                        @csrf
                                        <button type="submit" class="w-full sm:w-auto bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-xs px-3.5 py-2 rounded-xl transition shadow-xs cursor-pointer text-center">
                                            Setujui
                                        </button>
                                    </form>
                                    <form action="{{ route('kader.members.verify', [$m->id, 'ditolak']) }}" method="POST" class="flex-1 sm:flex-none" onsubmit="return confirm('Apakah Anda yakin ingin menolak pendaftaran ini?')">
                                        @csrf
                                        <button type="submit" class="w-full sm:w-auto bg-white hover:bg-rose-50 border border-slate-200 text-rose-600 text-xs font-bold px-3.5 py-2 rounded-xl transition cursor-pointer text-center">
                                            Tolak
                                        </button>
                                    </form>
                                @else
                                    @if($m->status_verifikasi === 'disetujui')
                                        ...
                                        <a href="{{ route('kader.health.check', $m->id) }}" 
                                            class="flex-1 sm:flex-none bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-xs px-3 py-2 rounded-xl transition shadow-xs flex items-center justify-center gap-1 whitespace-nowrap">
                                            <svg class="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 9v3m0 0v3m0-3h3m-3 0h-3m-9-4h18"></path></svg>
                                    ...
                                    <button onclick="openEditModal({{ $m->id }}, '{{ addslashes($m->nama) }}', '{{ $m->jenis_kelamin }}', '{{ $m->tanggal_lahir->format('Y-m-d') }}', '{{ $m->kategori }}')" 
                                        class="bg-white hover:bg-slate-100 border border-slate-200 text-slate-700 text-xs font-bold px-3 py-2 rounded-xl transition cursor-pointer">
                                        Edit
                                    </button>
                                    ...
                                    <form action="{{ route('kader.members.delete', $m->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus anggota keluarga ini?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="bg-white hover:bg-rose-50 border border-slate-200 text-rose-500 hover:text-rose-600 text-xs font-bold px-3 py-2 rounded-xl transition cursor-pointer">
                                            Hapus
                                        </button>
                                    </form>
                                @endif
    ...
    <!-- Add Member Panel -->
    <div>
        <div class="bg-white border border-slate-100 rounded-3xl p-5 sm:p-6 shadow-xs sticky top-20">
