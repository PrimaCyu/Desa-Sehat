                ...
                <div class="sticky top-20 bg-white md:rounded-2xl md:border md:border-slate-100 md:shadow-sm md:p-4 flex flex-col gap-1">
                    ...
                    @if(auth()->user()->isKader())
                        ...
                        @if(auth()->user()->isKaderUtama())
                            <a href="{{ route('kader.kaders.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 {{ request()->routeIs('kader.kaders.*') ? 'bg-emerald-500 text-white shadow-md shadow-emerald-100' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900' }}">
